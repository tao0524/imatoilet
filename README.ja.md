# 🚻 imatoilet

[English](README.md)

> 日本全国のトイレを、今すぐ見つける。清潔さ・バリアフリー設備・場所から絞り込めるトイレ検索Webアプリ。

<!-- 実際のスクリーンショットに差し替えてください -->
<!-- ![imatoilet スクリーンショット](docs/screenshot.png) -->

## 🔗 デモ

[デモURL](https://imatoilet.vercel.app)

---

## ✨ 主な機能

| 機能 | 説明 |
|---|---|
| 📍 **マップ検索** | Google Maps上でトイレをインタラクティブに表示。AdvancedMarker・MarkerClustererに対応 |
| 🗾 **全国スケール対応** | 地図の表示領域（Bounding Box）で動的にAPIを叩き、日本全国どこでも検索可能 |
| ♿ **バリアフリーフィルター** | 車椅子・おむつ台・24時間・オストメイト・授乳室・ウォシュレット・無料など13項目以上で絞り込み |
| ⭐ **レビュー・コメント** | 清潔度評価つきのテキストレビューを投稿可能 |
| ❤️ **お気に入り** | よく使うトイレをローカル保存してお気に入り一覧から参照 |
| 🗺️ **ルート案内** | 現在地からトイレまでの徒歩・車ルートをGoogle Mapsで表示 |
| ✏️ **管理者向けCRUD** | トークン認証による管理者専用のトイレ情報追加・編集・削除 |
| 📱 **PWA対応** | スマートフォンにアプリとしてインストール可能 |
| 🏙️ **オープンデータ活用** | つくば市バリアフリーマップのオープンデータおよび全国主要観光地のトイレ情報を収録 |

---

## 🛠 技術スタック

### バックエンド

| レイヤー | 技術 |
|---|---|
| 言語 | Java 21 (Eclipse Temurin) |
| フレームワーク | Spring Boot 3.5.9 |
| ORM | Spring Data JPA / Hibernate |
| DBマイグレーション | Flyway 11.7.2 |
| ローカルDB | H2（インメモリ・`dev`プロファイル） |
| 本番DB | PostgreSQL |
| ビルドツール | Apache Maven 3.9.12 |
| セキュリティ | Spring Security + カスタム `AdminTokenFilter` |
| テスト | JUnit 5 + MockMvc |

### フロントエンド

| レイヤー | 技術 |
|---|---|
| フレームワーク | React 19 + Vite |
| UIコンポーネント | MUI (Material-UI) v7 |
| 地図 | Google Maps JavaScript API（`@react-google-maps/api`） |
| マーカークラスタリング | `@googlemaps/markerclusterer` |
| 画像ホスティング | Cloudinary（オプション） |
| テスト | Vitest + Testing Library |

### アーキテクチャ

```
[React (Vite)]  ──REST API──>  [Spring Boot]  ──JPA──>  [PostgreSQL]
      │                                                   [H2 (dev)]
      │
  Google Maps JS API
  Cloudinary（画像）
```

---

## 💡 開発ストーリーと工夫した点

### 1. 将来を見据えたパフォーマンス対策と、AIを活用したトラブルシューティング（Google Maps API）
本プロジェクトでは、開発のサポートとして生成AIを積極的に活用しました。しかし、Google Maps APIのような仕様が複雑な領域では、AIの回答に古い情報や矛盾が含まれる（ハルシネーション）壁に何度も直面し、技術の難しさを痛感しました。

そのため、「AIの出したコードを丸写しするのではなく、エラーログや公式ドキュメントと見比べながら原因を探る」という姿勢を心がけました。

開発の中で特に意識したのは、将来的に全国の大量のトイレデータを扱うことを見据えた「パフォーマンス低下への予防線」です。現状のデータ量では問題ありませんが、将来データが増えた際に地図の描画が重くなるリスクを考慮し、AIにヒントをもらいつつ公式仕様を確認しながら、マーカーをグループ化して描画負荷を抑える「MarkerClusterer」の導入に挑戦しました。

まだまだ手探りの部分は多いですが、試行錯誤の末になんとか実装を完了させ、データ拡張時にもスムーズな操作性を保てる土台を作ることができました。この経験を通して「エラーから逃げずに原因を調べ、将来の運用も見据えて地道に形にしていく」という、開発の基礎的な姿勢を学ぶことができたと感じています。

### 2. 基礎を活かしたバックエンド構築と、初挑戦のReactにおけるAPI連携
本プロジェクトでは、バックエンドにJava (Spring Boot)、フロントエンドにReactを採用しました。Javaに関しては以前から基礎的な学習を行っていたため、ツールの使い方や処理の流れに馴染みがあり、データベース（PostgreSQL）の構築からAPIの作成まで、比較的落ち着いて全体像を把握しながら作業を進めることができました。

一方で、フロントエンドのReactは今回が全くの初挑戦であり、コンポーネントの概念などをゼロから学びながらの実装となりました。

開発の中で最も苦労したのは、この初挑戦のReactとJavaを繋ぐAPI通信の部分です。ここでもAIのサポートを活用しましたが、フロントエンドとバックエンドという異なる環境を跨ぐ処理のためか、AIのハルシネーション（データの受け渡し方や通信設定に関する誤ったコードの提示）が頻発し、非常に厄介な壁となりました。

AIが提示したReact側の通信コードをそのまま実行しても、Java側でうまくデータを受け取れずエラーになることが多々ありました。しかし、Javaの基礎知識があったおかげで「まずはバックエンド側が正しく動いているか確認しよう」と問題を切り分けて考えることができ、エラーの原因を冷静に探ることができました。

初めてのReactに戸惑うこともありましたが、AIに頼りきりになるのではなく、自らエラーログを読み解き、フロントエンドからデータベースまで一貫してデータが通る仕組みを構築できたことは、フルスタックな開発の流れを理解する上で非常に良い勉強になりました。

### 3. 原体験と競合分析から生まれた、ユーザーに寄り添うUI/UX設計
「imatoilet」は、「外出先でトイレだけをすぐに探したい」という私自身の実体験をきっかけに開発をスタートしました。お恥ずかしい話ですが、開発を始めてから世の中にすでに多くのトイレ検索アプリが存在することに気がつきました。

しかし、そこでただ作るのをやめるのではなく、「既存のアプリはどのような機能が便利で、逆に何が使いにくいのか」を実際に複数ダウンロードして使い比べ、分析を行いました。その結果、私がこのアプリで最もこだわったのは「高齢者やスマートフォン操作に不慣れな方でも、迷わず直感的に使えるシンプルさ」です。

多機能になりすぎて画面が複雑になることを避け、本当に必要な情報（バリアフリー設備や現在地からの距離など）にすぐアクセスできるようUIを工夫しました。また、外出先でトイレを探している時の「焦り」や「不安」を少しでも和らげたいという思いから、アプリ全体のデザインや配色は、無機質なものではなく柔らかく優しい雰囲気を持たせるよう心がけています。

私はまだエンジニアとしての実務経験はありませんが、今回の個人開発を通して、技術的な実装を形にするだけでなく「誰が、どんな気持ちでこのアプリを開くのか」を想像することの大切さを学びました。これから実務経験を積み、プロのエンジニアとして成長していく上でも、このユーザー視点を常に忘れずに開発に向き合っていきたいと考えています。

## 🚀 ローカル開発環境のセットアップ

### 必要なもの

- Java 21
- Apache Maven 3.9.12（直接インストール — **`mvnw` は使用しないこと**）
- Node.js 18以上
- Google Maps APIキー

### 1. リポジトリをクローン

```bash
git clone https://github.com/tao0524/imatoilet.git
cd imatoilet
```

### 2. バックエンドを起動（H2インメモリ・devプロファイル）

```powershell
cd backend\backend
mvn clean spring-boot:run "-Dspring-boot.run.profiles=dev"
```

バックエンド起動URL: `http://localhost:8080`

**H2コンソール**（ローカル開発時のみ）:

| 項目 | 値 |
|---|---|
| アクセスURL | `http://localhost:8080/h2-console` |
| JDBC URL | `jdbc:h2:mem:imatoiletdb` |
| ユーザー名 | `sa` |
| パスワード | *（空欄）* |

### 3. フロントエンドの環境変数を設定

```bash
cd toilet-frontend
cp .env.example .env
# VITE_GOOGLE_MAPS_API_KEY などを記入してください
```

### 4. フロントエンドを起動

```bash
npm install
npm run dev
```

フロントエンド起動URL: `http://localhost:5173`

---

## 🗄 データベースマイグレーション

Flywayマイグレーションは起動時に自動実行されます。
マイグレーションスクリプトの格納場所:

```
backend/src/main/resources/db/migration/       # PostgreSQL（本番）
backend/src/main/resources/db/migration-h2/    # H2（ローカル開発）
```

主なマイグレーション:

| バージョン | 内容 |
|---|---|
| V1 | 初期スキーマ（toilets・equipment） |
| V4 | サンプルデータ投入 |
| V9 | つくば市オープンデータ投入 |
| V11 | レビューテーブル追加 |
| V13〜V15 | 東京密集データ・観光地・全国スケールデータ追加 |

---

## 🔌 APIエンドポイント

| メソッド | エンドポイント | 説明 |
|---|---|---|
| `GET` | `/api/toilets` | トイレ検索（位置情報・Bounding Box・キーワード・フィルター） |
| `GET` | `/api/toilets/{id}` | トイレ詳細取得 |
| `POST` | `/api/toilets` | トイレ追加 *（管理者トークン必須）* |
| `PUT` | `/api/toilets/{id}` | トイレ更新 *（管理者トークン必須）* |
| `DELETE` | `/api/toilets/{id}` | トイレ削除 *（管理者トークン必須）* |
| `GET` | `/api/toilets/{id}/reviews` | レビュー一覧取得 |
| `POST` | `/api/toilets/{id}/reviews` | レビュー投稿 |

### 主な検索パラメータ

| パラメータ | 型 | 説明 |
|---|---|---|
| `lat` / `lng` | `Double` | 現在地座標（半径検索） |
| `radius` | `Double` | 検索半径（km、デフォルト: 5.0） |
| `minLat/maxLat/minLng/maxLng` | `Double` | Bounding Box（全国スケール検索） |
| `keyword` | `String` | 名称・住所のキーワード検索 |
| `facilityCategory` | `String` | `station`・`park`・`commercial`・`public` など |
| `equipment` | `List<String>` | `WHEELCHAIR`・`DIAPER`・`OPEN_24H`・`OSTOMATE` など |

---

## 🧪 テストの実行

```bash
# バックエンド
cd backend/backend
mvn test

# フロントエンド
cd toilet-frontend
npm run test
```

---

## 📁 ディレクトリ構成

```
imatoilet/
├── backend/backend/           # Spring Bootアプリケーション
│   ├── src/main/java/         # Controller・Service・Entity・Repository
│   ├── src/main/resources/    # application.properties・Flywayマイグレーション
│   └── src/test/              # ユニット・統合テスト
└── toilet-frontend/           # React + Viteアプリケーション
    ├── src/
    │   ├── components/        # 共有UIコンポーネント（SafeGoogleMap・ToiletCardなど）
    │   ├── hooks/             # カスタムフック（useToiletSearch）
    │   ├── pages/             # ルートページ（Home・Search・Detail・Register・Favorites）
    │   └── utils.js           # ユーティリティ（距離計算・設備情報正規化）
    └── public/                # PWAアイコン・マニフェスト
```

---

## 📝 ライセンス

MIT

---

*ポートフォリオとして個人開発しています。*