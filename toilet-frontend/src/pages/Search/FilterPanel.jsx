...
function FilterPanel({
  ...
  const updateParam = (key, value) => {
    ...
    if (value === null || value === false || value === '') newParams.delete(key);
      ...
      <div className={`accordion-content ${isOpen ? 'open' : ''}`}>
        ...
        <section className="filter-section">
          ...
          <div className="radio-group-vertical">
            <label className={`radio-row ${getStrParam('facilityCategory') === '' ? 'active' : ''}`}>
              <input type="radio" name="cat" checked={getStrParam('facilityCategory') === ''} onChange={() => updateParam('facilityCategory', '')} />
            ...
            {[
              ...
              <label key={opt.val} className={`radio-row ${getStrParam('facilityCategory') === opt.val ? 'active' : ''}`}>
                <input type="radio" name="cat" checked={getStrParam('facilityCategory') === opt.val} onChange={() => updateParam('facilityCategory', opt.val)} />
        ...
        <section className="filter-section" style={{ borderBottom: 'none', marginBottom: 0 }}>
          <div className="check-group-vertical">
            ...
            {/* ★追加: ベビーチェア */}
            <CheckRow label="ベビーチェア"   pKey="baby_chair"       icon="🪑" checked={getBoolParam('baby_chair')}       onChange={updateParam} />
        ...
        <button className="mobile-toggle-btn btn-bottom" onClick={() => setIsOpen(false)}>
          ▲ 詳細条件を閉じる
        </button>
      </div>
    </aside>
...
function CheckRow({ label, pKey, icon, checked, onChange }) {
  return (
    <label className={`check-row ${checked ? 'checked' : ''}`}>
      ...
      <input type="checkbox" checked={checked} onChange={(e) => onChange(pKey, e.target.checked)} />
